...
void main() {
  ...
  setUp(() {
    ...
    when(mockGetWatchListStatus.execute(0)).thenAnswer((_) async => false);

    when(mockGetWatchListStatus.execute(1)).thenAnswer((_) async => false);

    when(mockSaveWatchlist.execute(testMovieDetail1))
        .thenAnswer((_) async => Right('Added to Watchlist'));

    when(mockRemoveWatchlist.execute(testMovieDetail1))
